...
//   useEffect(() => {
//     // List of handles you want to check in the contest standings
//     const handles = [
//       "hewhocodes",
//       "sohamrane301",
//       "amalverma",
...
//     // Contest ID you want to fetch standings for
//     const contestId = 2032;

//     // Fetch the contest standings from Codeforces API
//     fetch(
...
//       .then((data) => {
//         if (data.status === "OK") {
//           // Filter rows to get participants who scored points in the contest
//           const participants = data.result.rows
//             .filter((row) => row.points > 0) // Filter participants with non-zero points
//             .map((row) => row.party.members[0].handle); // Get each participant's handle

//           console.log("Participants who gave the contest:", participants);
//         } else {
  ...
  useEffect(() => {
    ...
    const fetchRatingsAndAttendance = async () => {
      ...
      try {
        ...
        // Initialize attendance map for each user with false values
        const attendanceMap = handles.reduce((acc, handle) => {
          acc[handle] = [false, false, false, false, false]; // 5 contests, all initially false
          return acc;
        }, {});
        ...
        for (const contestId of lastFiveContests) {
          ...
          // Update attendance map for participants who scored points
          response.data.result.rows
            ...
            .forEach((row) => {
              ...
              if (attendanceMap[handle]) {
                attendanceMap[handle][contestIndex] = true; // Mark as attended for that contest
              }
            });
        }
        setAttendanceMap(attendanceMap);

        // Fetch ratings and finalize each member's data
        for (const [username, data] of Object.entries(members)) {
          const cfData = await fetchUserRating(data.cf_username);
          const leetCodeRating = await fetchLeetCodeRating(data.lc_username);
          ...
          <tbody>
            {leaderboardData.map((member, index) => (
              ...
              >
                ...
                <td className="p-2 sm:p-4 flex space-x-2 items-center justify-center">
                  {member.cf_username in attendanceMap &&
